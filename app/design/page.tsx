...
type ActiveTool = 'text'|'upload'|'ai'|'gfx'|'shirt'|'order';

// ─── Constants ────────────────────────────────────────────────
const SVG_W = 200, SVG_H = 230;
const PRINT = { x:60, y:85, w:80, h:105 };
// Improved shirt path with smoother curves
...
const FONTS = [
  ...
  {id:"'Bebas Neue',Impact,sans-serif",    label:'Display'},
];
const EMOJIS = ['🔥','⚡','💀','🎭','🌊','🦁','🎨','🎵','🏆','💎','🌙','⭐','🚀','🎯','🐉','👑','☠','✦','★','◆'];
const TEXT_COLORS = ['#ffffff','#000000','#FF4D1C','#FFD700','#10B981','#6C63FF','#FF69B4','#00BCD4','#F97316','#8B5CF6'];
...
function DesignStudio() {
  ...
  // Single active tool controls both left sidebar and right panel
  ...
  useEffect(() => {
    function onKey(e:KeyboardEvent) {
      ...
      if(selected&&notInput) {
        ...
        if(e.key==='ArrowLeft')  {e.preventDefault();updateLayer(selected,{x:Math.max(0,  (layers.find(l=>l.id===selected)?.x??50)-s)});}
        if(e.key==='ArrowRight') {e.preventDefault();updateLayer(selected,{x:Math.min(100,(layers.find(l=>l.id===selected)?.x??50)+s)});}
        if(e.key==='ArrowUp')    {e.preventDefault();updateLayer(selected,{y:Math.max(0,  (layers.find(l=>l.id===selected)?.y??50)-s)});}
        if(e.key==='ArrowDown')  {e.preventDefault();updateLayer(selected,{y:Math.min(100,(layers.find(l=>l.id===selected)?.y??50)+s)});}
      }
    }
  ...
  function renderLayers(interactive=true) {
    return layers.map(layer => {
      ...
      const aw=layer.type==='text'?layer.content.length*layer.fontSize*0.58:layer.fontSize*1.1, ah=layer.fontSize*1.3;
      return (
        <g key={layer.id} transform={`translate(${lx},${ly}) rotate(${layer.rotation})`}
          style={interactive?{cursor:'move'}:{}} onPointerDown={interactive?(e)=>onLayerDown(e,layer.id):undefined}>
          ...
          <text textAnchor="middle" dominantBaseline="middle" fontSize={layer.fontSize}
            ...
            fontStyle={layer.italic?'italic':'normal'} style={{userSelect:'none'}}>{layer.content}</text>
          {isSel && <>
            <rect x={-aw/2-5} y={-ah/2-3} width={aw+10} height={ah+6} fill="rgba(0,229,200,0.07)" stroke="#00E5C8" strokeWidth="0.8" strokeDasharray="2.5,1.5" rx="2"/>
  ...
  function ShirtCanvas({w,h,interactive=true}:{w:number;h:number;interactive?:boolean}) {
    return (
      <svg width={w} height={h} viewBox={`0 0 ${SVG_W} ${SVG_H}`} fill="none" style={{overflow:'visible'}}>
        <defs>
          <filter id="shirtShadow" x="-30%" y="-10%" width="160%" height="140%">
          ...
          <filter id="platformBlur"><feGaussianBlur stdDeviation="6"/></filter>
          <linearGradient id="shirtGrad" x1="0.18" y1="0" x2="0.82" y2="1">
            <stop offset="0%"   stopColor={color.hex} stopOpacity="1"/>
            <stop offset="50%"  stopColor={color.hex} stopOpacity="0.97"/>
            <stop offset="100%" stopColor={color.hex} stopOpacity="0.93"/>
          </linearGradient>
          <linearGradient id="shirtHighlight" x1="0.12" y1="0" x2="0.88" y2="0.55">
            <stop offset="0%"   stopColor="rgba(255,255,255,0.22)"/>
            <stop offset="45%"  stopColor="rgba(255,255,255,0.05)"/>
            <stop offset="100%" stopColor="rgba(255,255,255,0)"/>
          </linearGradient>
          <radialGradient id="shirtSpotlight" cx="48%" cy="18%" r="65%">
            <stop offset="0%"   stopColor="rgba(255,255,255,0.12)"/>
            <stop offset="70%"  stopColor="rgba(255,255,255,0.02)"/>
            <stop offset="100%" stopColor="rgba(255,255,255,0)"/>
          </radialGradient>
          <linearGradient id="leftSleeveShadow" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%"  stopColor="rgba(0,0,0,0.15)"/>
            <stop offset="100%" stopColor="rgba(0,0,0,0)"/>
          </linearGradient>
          <linearGradient id="rightSleeveShadow" x1="1" y1="0" x2="0" y2="0">
        ...
        {/* Platform shadow */}
        <ellipse cx="100" cy="228" rx="68" ry="6.5" fill="rgba(0,0,0,0.28)" filter="url(#platformBlur)"/>

        {/* Main shirt */}
        <path d={SHIRT_PATH} fill="url(#shirtGrad)" filter="url(#shirtShadow)"/>

        {/* Sleeve fold shadows */}
        <path d="M30 58 C18 65,2 79,2 82 L26 97 L30 97 C28 80,20 68,30 58Z" fill="url(#leftSleeveShadow)"/>
        <path d="M170 58 C182 65,198 79,198 82 L174 97 L170 97 C172 80,180 68,170 58Z" fill="url(#rightSleeveShadow)"/>

        {/* Collar shadow */}
        <path d="M56 72 Q70 52,100 50 Q130 52,144 72 Q130 60,100 58 Q70 60,56 72Z" fill="rgba(0,0,0,0.18)"/>

        {/* Seam details */}
        <path d="M30 58 C18 65,2 79,2 82 L26 97" fill="none" stroke="rgba(0,0,0,0.08)" strokeWidth="1.5" strokeLinecap="round"/>
        <path d="M170 58 C182 65,198 79,198 82 L174 97" fill="none" stroke="rgba(0,0,0,0.06)" strokeWidth="1.5" strokeLinecap="round"/>
        ...
        {/* Surface highlight (studio light) */}
        <path d={SHIRT_PATH} fill="url(#shirtHighlight)"/>
        {/* Spot highlight */}
        <path d={SHIRT_PATH} fill="url(#shirtSpotlight)"/>
        ...
        {interactive&&layers.length===0&&!activeImg&&!aiSvg&&(
          <rect x={PRINT.x} y={PRINT.y} width={PRINT.w} height={PRINT.h}
            fill={isLight?'rgba(0,0,0,0.025)':'rgba(255,255,255,0.035)'}
            stroke={isLight?'rgba(0,0,0,0.18)':'rgba(255,255,255,0.22)'}
            strokeDasharray="3,2" rx="3" strokeWidth="0.85"/>
        )}
  ...
  if (ordered) return (
    <div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(180deg,#050507,#060610)',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{textAlign:'center',maxWidth:400,position:'relative',zIndex:1,padding:'0 24px'}}>
        ...
        <p style={{fontSize:'0.78rem',color:'rgba(255,255,255,0.3)',marginBottom:28,lineHeight:1.6}}>Your shirt is being prepared. Track it on your <Link href="/profile" style={{color:'#00E5C8',textDecoration:'none'}}>profile</Link>.</p>
        <div style={{display:'flex',gap:10,justifyContent:'center'}}>
          <button onClick={()=>{setOrdered(false);setLayers([]);setAiSvg(null);setUploads({front:null,back:null,chest:null});}} style={{padding:'0.85rem 1.5rem',borderRadius:12,border:'1px solid rgba(255,255,255,0.12)',background:'rgba(255,255,255,0.05)',color:'rgba(255,255,255,0.75)',fontWeight:700,cursor:'pointer',fontSize:'0.85rem'}}>Design another</button>
  ...
  const fullscreenModal = fullscreen && (
    <div style={{position:'fixed',inset:0,zIndex:9999,background:'radial-gradient(ellipse at 50% 38%,rgba(14,14,24,1) 0%,rgba(4,4,6,1) 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',animation:'fsIn 0.25s ease'}}
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:'#070709',color:'white',overflow:'hidden'}}>
      ...
      <header style={{height:50,flexShrink:0,borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',padding:'0 16px 0 12px',gap:12,background:'rgba(7,7,9,0.98)',backdropFilter:'blur(24px)',position:'relative',zIndex:20}}>
        ...
        <Link href="/catalog" style={{display:'flex',alignItems:'center',gap:4,color:'rgba(255,255,255,0.28)',fontSize:'0.67rem',textDecoration:'none',fontWeight:700,letterSpacing:'0.06em',transition:'color 0.15s',textTransform:'uppercase',flexShrink:0,paddingRight:4}}
        ...
        <h1 style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontWeight:400,fontSize:'1.42rem',letterSpacing:'0.1em',lineHeight:1,margin:0,flex:1}}>
          DESIGN<span style={{color:'#00E5C8'}}>.</span>STUDIO
        </h1>

        {/* Center: color + size indicator */}
        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center',gap:6,padding:'4px 10px 4px 6px',borderRadius:20,border:'1px solid rgba(255,255,255,0.08)',background:'rgba(255,255,255,0.03)'}}>
            <span style={{width:16,height:16,borderRadius:'50%',background:color.hex,display:'inline-block',flexShrink:0,outline:'1px solid rgba(255,255,255,0.15)',outlineOffset:1}}/>
            <span style={{fontSize:'0.65rem',fontWeight:600,color:'rgba(255,255,255,0.45)'}}>{color.name}</span>
          </div>
          {size ? (
            <div style={{padding:'4px 10px',borderRadius:20,border:'1px solid rgba(0,229,200,0.22)',background:'rgba(0,229,200,0.07)',fontSize:'0.65rem',fontWeight:700,color:'#00E5C8'}}>{size}</div>
          ) : (
            <div style={{padding:'4px 10px',borderRadius:20,border:'1px solid rgba(255,255,255,0.06)',background:'rgba(255,255,255,0.02)',fontSize:'0.65rem',fontWeight:600,color:'rgba(255,255,255,0.22)',cursor:'pointer'}} onClick={()=>setActiveTool('shirt')}>+ Size</div>
          )}
          {layers.length>0&&<div style={{padding:'4px 10px',borderRadius:20,border:'1px solid rgba(0,229,200,0.18)',background:'rgba(0,229,200,0.06)',fontSize:'0.62rem',fontWeight:700,color:'rgba(0,229,200,0.8)'}}>{layers.length}L</div>}
        </div>

        <button onClick={()=>setFullscreen(true)}
          style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:9,padding:'5px 13px',color:'rgba(255,255,255,0.35)',fontSize:'0.64rem',fontWeight:700,cursor:'pointer',letterSpacing:'0.06em',transition:'all 0.15s',flexShrink:0}}
      ...
      {/* ── 3-PANEL BODY ─────────────────────────────────────────── */}
      <div style={{flex:1,display:'grid',gridTemplateColumns:'64px 1fr 330px',overflow:'hidden',minHeight:0}}>

        {/* ── LEFT SIDEBAR ─────────────────────────────────────── */}
        <div style={{background:'rgba(5,5,8,1)',borderRight:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',alignItems:'center',padding:'12px 0',gap:2,zIndex:10}}>
          {/* Design tools */}
          {TOOLS.map(t=>(
            <button key={t.id} title={t.label} onClick={()=>setActiveTool(t.id)}
              style={{
                width:48, height:48, borderRadius:12, border:'none', cursor:'pointer',
                background:activeTool===t.id?'rgba(0,229,200,0.12)':'transparent',
                color:activeTool===t.id?'#00E5C8':'rgba(255,255,255,0.28)',
            ...
            >
              {/* Active indicator */}
              {activeTool===t.id&&<div style={{position:'absolute',left:0,top:'20%',bottom:'20%',width:2,borderRadius:'0 2px 2px 0',background:'#00E5C8'}}/>}
              <span style={{
                fontSize: t.id==='text'?'0.9rem':'0.85rem',
                fontWeight:t.id==='text'?900:700,
                fontFamily:t.id==='text'?'Georgia,serif':'inherit',
                lineHeight:1,
              }}>{t.icon}</span>
              <span style={{fontSize:'0.48rem',fontWeight:700,letterSpacing:'0.06em',textTransform:'uppercase'}}>{t.label}</span>
            </button>
          ))}
          ...
          {([{id:'shirt' as ActiveTool,icon:'👕',label:'Shirt'},{id:'order' as ActiveTool,icon:'🛒',label:'Order'}]).map(t=>(
            ...
            >
              {activeTool===t.id&&<div style={{position:'absolute',left:0,top:'20%',bottom:'20%',width:2,borderRadius:'0 2px 2px 0',background:'#00E5C8'}}/>}
              {t.id==='order'&&canOrder&&<div style={{position:'absolute',top:8,right:8,width:6,height:6,borderRadius:'50%',background:'#00E5C8',boxShadow:'0 0 6px rgba(0,229,200,0.8)'}}/>}
        ...
        {/* ── CANVAS ───────────────────────────────────────────── */}
        <div style={{background:'radial-gradient(ellipse at 50% 35%,rgba(13,13,22,1) 0%,rgba(5,5,8,1) 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}
          ...
          {[[0,0,'Top','Left'],[0,1,'Top','Right'],[1,0,'Bottom','Left'],[1,1,'Bottom','Right']].map(([r,c,v,h],i)=>(
            <div key={i} style={{position:'absolute',
              ...
              width:18,height:18,
              borderTop:v==='Top'?'1px solid rgba(0,229,200,0.18)':'none',
              borderBottom:v==='Bottom'?'1px solid rgba(0,229,200,0.18)':'none',
              borderLeft:h==='Left'?'1px solid rgba(0,229,200,0.18)':'none',
              borderRight:h==='Right'?'1px solid rgba(0,229,200,0.18)':'none',
              pointerEvents:'none',
            }}/>
          ))}

          {/* Front/Back toggle */}
          <div style={{position:'absolute',top:16,left:16,background:'rgba(5,5,8,0.9)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:12,padding:3,display:'flex',gap:2,backdropFilter:'blur(14px)',zIndex:5}}>
            {['Front','Back'].map(s=>(
              <button key={s} aria-pressed={(s==='Back')===showBack}
                onClick={e=>{e.stopPropagation();setShowBack(s==='Back');}}
                style={{padding:'5px 14px',borderRadius:9,border:'none',cursor:'pointer',background:(s==='Back')===showBack?'rgba(0,229,200,0.13)':'transparent',color:(s==='Back')===showBack?'#00E5C8':'rgba(255,255,255,0.3)',fontSize:'0.62rem',fontWeight:700,letterSpacing:'0.07em',transition:'all 0.15s'}}>
                {s.toUpperCase()}
              </button>
            ))}
          </div>
          ...
          <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',flex:1,width:'100%'}}>
            {/* Shirt glow based on color */}
            <div style={{position:'absolute',width:380,height:420,borderRadius:'50%',background:`radial-gradient(ellipse,${color.hex}0a 0%,transparent 60%)`,pointerEvents:'none',filter:'blur(20px)'}}/>

            <div ref={canvasRef} role="img" aria-label="Shirt design canvas — drag elements to position them"
              style={{
                position:'relative',
                filter:`drop-shadow(0 45px 90px rgba(0,0,0,0.55)) drop-shadow(0 8px 20px rgba(0,0,0,0.35))`,
            ...
            {/* Selected layer bar */}
            {selLayer&&(
              <div style={{position:'absolute',bottom:10,left:'50%',transform:'translateX(-50%)',background:'rgba(4,4,7,0.95)',border:'1px solid rgba(0,229,200,0.25)',borderRadius:12,padding:'7px 16px',display:'flex',alignItems:'center',gap:10,backdropFilter:'blur(16px)',fontSize:'0.67rem',animation:'fadeUp 0.15s ease',boxShadow:'0 4px 20px rgba(0,229,200,0.08)',whiteSpace:'nowrap',zIndex:6}}>
                ...
                <span style={{color:'rgba(255,255,255,0.2)',fontSize:'0.6rem'}}>Drag · Arrows · Del</span>
                <button onClick={e=>{e.stopPropagation();deleteLayer(selLayer.id);}} style={{background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:6,padding:'3px 8px',color:'#f87171',fontSize:'0.58rem',fontWeight:700,cursor:'pointer'}}>✕</button>
              </div>
            )}

            {/* Empty state */}
            {layers.length===0&&!activeImg&&!aiSvg&&!showBack&&(
              <div style={{position:'absolute',bottom:10,textAlign:'center',pointerEvents:'none',animation:'fadeUp 0.5s ease 0.4s both'}}>
                <p style={{color:'rgba(255,255,255,0.1)',fontSize:'0.63rem',letterSpacing:'0.12em',textTransform:'uppercase'}}>Use the tools on the left to design your shirt</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{borderLeft:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',background:'rgba(8,8,12,1)',overflow:'hidden'}}>
          ...
          {/* Panel header */}
          <div style={{height:46,flexShrink:0,borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',padding:'0 16px',gap:8}}>
            <span style={{fontSize:'0.58rem',fontWeight:700,color:'rgba(255,255,255,0.18)',letterSpacing:'0.14em',textTransform:'uppercase',flex:1}}>
              {activeTool==='text'&&'TEXT TOOL'}
              {activeTool==='upload'&&'UPLOAD IMAGE'}
              {activeTool==='ai'&&'AI GENERATOR'}
              {activeTool==='gfx'&&'GRAPHICS'}
              {activeTool==='shirt'&&'SHIRT SETTINGS'}
              {activeTool==='order'&&'PLACE ORDER'}
            </span>
            {activeTool==='shirt'&&!size&&<span style={{fontSize:'0.58rem',color:'rgba(245,158,11,0.6)',fontWeight:600}}>Select size ↓</span>}
            {activeTool==='order'&&canOrder&&<span style={{fontSize:'0.58rem',color:'rgba(0,229,200,0.7)',fontWeight:700}}>Ready ✓</span>}
          </div>
          ...
          <div ref={panelRef} style={{flex:1,overflowY:'auto',minHeight:0}}>
              ...
              <div style={{padding:'16px'}}>
                ...
                <div style={{marginBottom:18}}>
                  <div style={{display:'flex',gap:7,marginBottom:9}}>
                    <input aria-label="Text to add" value={textInput} onChange={e=>setTextInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addText()} placeholder="Type your text..." maxLength={40}
                      style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1.5px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'10px 13px',color:'white',fontSize:'0.88rem',outline:'none',fontFamily:fontFam,fontWeight:fontWeight==='bold'?'bold':'normal',fontStyle:italic?'italic':'normal',transition:'border-color 0.15s'}}
                    ...
                    <button onClick={addText} disabled={!textInput.trim()}
                      style={{width:44,height:44,borderRadius:10,border:'none',background:textInput.trim()?'linear-gradient(135deg,#00E5C8,#0099FF)':'rgba(255,255,255,0.06)',color:textInput.trim()?'#050507':'rgba(255,255,255,0.15)',fontSize:'1.25rem',fontWeight:700,cursor:textInput.trim()?'pointer':'default',transition:'all 0.15s',flexShrink:0}}>+</button>
                  </div>
                  {/* Quick presets */}
                  <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                    {['YOUR NAME','EST. 2025','ORIGINAL','NO RULES','MADE IN'].map(t=>(
                      <button key={t} onClick={()=>setTextInput(t)}
                        style={{padding:'4px 10px',borderRadius:20,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',color:'rgba(255,255,255,0.3)',fontSize:'0.6rem',fontWeight:700,cursor:'pointer',letterSpacing:'0.04em',transition:'all 0.12s'}}
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:4}}>
                    {FONTS.map(f=>(
                      <button key={f.id} aria-pressed={fontFam===f.id} onClick={()=>{setFontFam(f.id);if(selected)updateLayer(selected,{fontFamily:f.id});}}
                        style={{padding:'9px 12px',borderRadius:9,cursor:'pointer',background:fontFam===f.id?'rgba(0,229,200,0.09)':'rgba(255,255,255,0.02)',border:`1px solid ${fontFam===f.id?'rgba(0,229,200,0.3)':'rgba(255,255,255,0.06)'}`,color:fontFam===f.id?'#00E5C8':'rgba(255,255,255,0.38)',fontSize:'0.85rem',fontWeight:600,fontFamily:f.id,transition:'all 0.13s',textAlign:'left',display:'flex',alignItems:'center',gap:8}}>
                        {fontFam===f.id&&<span style={{width:4,height:4,borderRadius:'50%',background:'#00E5C8',flexShrink:0}}/>}
                        <span style={{fontFamily:f.id,fontSize:'0.95rem'}}>{f.label}</span>
                        <span style={{marginLeft:'auto',fontSize:'0.65rem',opacity:0.5,fontFamily:'system-ui'}}>Aa</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:12,marginBottom:16,alignItems:'end'}}>
                  <div>
                    ...
                    <input type="range" aria-label="Font size" min={8} max={72} value={fontSize} onChange={e=>{const v=+e.target.value;setFontSize(v);if(selected)updateLayer(selected,{fontSize:v});}} style={{width:'100%',accentColor:'#00E5C8'}}/>
                  </div>
                  <div>
                    ...
                    <div style={{display:'flex',gap:4}}>
                      <button aria-pressed={fontWeight==='bold'} onClick={()=>{const n:typeof fontWeight=fontWeight==='bold'?'normal':'bold';setFontWeight(n);if(selected)updateLayer(selected,{fontWeight:n});}}
                        style={{width:38,height:36,borderRadius:8,cursor:'pointer',background:fontWeight==='bold'?'rgba(0,229,200,0.1)':'rgba(255,255,255,0.04)',border:`1.5px solid ${fontWeight==='bold'?'rgba(0,229,200,0.32)':'rgba(255,255,255,0.08)'}`,color:fontWeight==='bold'?'#00E5C8':'rgba(255,255,255,0.3)',fontWeight:'bold',fontSize:'0.9rem',transition:'all 0.13s'}}>B</button>
                      <button aria-pressed={italic} onClick={()=>{const n=!italic;setItalic(n);if(selected)updateLayer(selected,{italic:n});}}
                        style={{width:38,height:36,borderRadius:8,cursor:'pointer',background:italic?'rgba(0,229,200,0.1)':'rgba(255,255,255,0.04)',border:`1.5px solid ${italic?'rgba(0,229,200,0.32)':'rgba(255,255,255,0.08)'}`,color:italic?'#00E5C8':'rgba(255,255,255,0.3)',fontStyle:'italic',fontWeight:700,fontSize:'0.9rem',transition:'all 0.13s'}}>I</button>
                    </div>
                  </div>
                </div>
                  ...
                  <div style={{display:'flex',gap:7,flexWrap:'wrap',alignItems:'center',marginBottom:8}}>
                    {TEXT_COLORS.map(c=>(
                      <button key={c} aria-label={`Color ${c}`} aria-pressed={textColor===c} onClick={()=>{setTextColor(c);if(selected)updateLayer(selected,{color:c});}}
                        style={{width:30,height:30,borderRadius:'50%',border:'none',background:c,cursor:'pointer',outline:textColor===c?'2.5px solid #00E5C8':'2px solid rgba(255,255,255,0.1)',outlineOffset:2,transition:'all 0.12s',transform:textColor===c?'scale(1.2)':'scale(1)',boxShadow:textColor===c?`0 0 10px ${c}88`:'none'}}/>
                    ))}
                    <label style={{width:30,height:30,borderRadius:'50%',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.06)',border:'1.5px dashed rgba(255,255,255,0.2)',fontSize:'0.8rem',position:'relative',color:'rgba(255,255,255,0.5)'}}>
                ...
                {/* Rotation (when layer selected) */}
                {selLayer&&selLayer.type==='text'&&(
                  <div style={{background:'rgba(0,229,200,0.04)',border:'1px solid rgba(0,229,200,0.1)',borderRadius:10,padding:'10px 12px'}}>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:3}}>
                      {[...layers].reverse().map(l=>(
                        <div key={l.id} onClick={()=>setSelected(l.id)}
                          style={{display:'flex',alignItems:'center',gap:8,padding:'6px 9px',borderRadius:8,cursor:'pointer',background:selected===l.id?'rgba(0,229,200,0.08)':'rgba(255,255,255,0.02)',border:`1px solid ${selected===l.id?'rgba(0,229,200,0.2)':'rgba(255,255,255,0.04)'}`,transition:'all 0.12s'}}>
                          <span style={{fontSize:'0.72rem',width:14,textAlign:'center',flexShrink:0}}>{l.type==='text'?'T':l.content}</span>
            ...
            {activeTool==='upload'&&(
              <div style={{padding:'16px'}}>
                ...
                <div onDragEnter={e=>{e.preventDefault();setFileDragging(true);}} onDragLeave={()=>setFileDragging(false)} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();setFileDragging(false);const f=e.dataTransfer.files[0];if(f)handleFile(f);}} onClick={()=>fileRef.current?.click()}
                  style={{border:`2px dashed ${fileDragging?'rgba(0,229,200,0.6)':uploads[uploadSlot]?'rgba(16,185,129,0.4)':'rgba(255,255,255,0.1)'}`,borderRadius:14,padding:uploads[uploadSlot]?'1.2rem 1rem':'2.5rem 1rem',textAlign:'center',cursor:'pointer',background:fileDragging?'rgba(0,229,200,0.05)':uploads[uploadSlot]?'rgba(16,185,129,0.03)':'rgba(255,255,255,0.01)',transition:'all 0.2s',marginBottom:12}}>
                  ...
                  {uploads[uploadSlot]
                    ?<div style={{display:'flex',alignItems:'center',gap:12,justifyContent:'center'}}><img src={uploads[uploadSlot]!} alt="upload" style={{height:60,maxWidth:110,borderRadius:8,objectFit:'contain'}}/><div><div style={{fontSize:'0.72rem',color:'#10B981',fontWeight:700}}>✓ Uploaded</div><div style={{fontSize:'0.62rem',color:'rgba(255,255,255,0.25)',marginTop:3}}>Click to replace</div></div></div>
                ...
                {uploadSlot!=='chest'&&(
                  <div style={{marginBottom:12}}>
                    <div style={LS}>Position on shirt</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5}}>
                      {(Object.keys(POS_LABELS) as ImagePos[]).map(p=>(
                        <button key={p} aria-pressed={imgPos[uploadSlot==='front'?'front':'back']===p} onClick={()=>setImgPos(prev=>({...prev,[uploadSlot==='front'?'front':'back']:p}))}
                          style={{padding:'8px 10px',borderRadius:9,border:'1px solid',borderColor:imgPos[uploadSlot==='front'?'front':'back']===p?'rgba(0,229,200,0.4)':'rgba(255,255,255,0.07)',background:imgPos[uploadSlot==='front'?'front':'back']===p?'rgba(0,229,200,0.08)':'rgba(255,255,255,0.02)',color:imgPos[uploadSlot==='front'?'front':'back']===p?'#00E5C8':'rgba(255,255,255,0.3)',fontSize:'0.65rem',fontWeight:600,cursor:'pointer',transition:'all 0.13s',textAlign:'center'}}>
                          {POS_LABELS[p]}
                        </button>
                      ))}
                    </div>
                ...
                {uploads[uploadSlot]&&<button onClick={()=>setUploads(p=>({...p,[uploadSlot]:null}))} style={{width:'100%',padding:'9px',borderRadius:9,border:'1px solid rgba(239,68,68,0.2)',background:'rgba(239,68,68,0.07)',color:'#f87171',fontSize:'0.7rem',fontWeight:700,cursor:'pointer'}}>🗑 Remove image</button>}
              </div>
            )}
            ...
            {activeTool==='ai'&&(
              <div style={{padding:'16px'}}>
                <div style={{background:'linear-gradient(135deg,rgba(0,229,200,0.06),rgba(0,153,255,0.05))',border:'1px solid rgba(0,229,200,0.12)',borderRadius:14,padding:'14px',marginBottom:16}}>
                  <div style={{fontSize:'0.6rem',color:'#00E5C8',fontWeight:700,letterSpacing:'0.1em',marginBottom:6}}>✦ AI DESIGN GENERATOR</div>
                  <div style={{fontSize:'0.73rem',color:'rgba(255,255,255,0.35)',lineHeight:1.6}}>Describe your ideal design and our AI will match it from the catalog.</div>
                </div>

                <div style={{marginBottom:12}}>
                  <div style={LS}>Your idea</div>
                  <textarea value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} placeholder="e.g. A minimalist mountain peak with bold EXPLORE typography..." rows={4} maxLength={200} aria-label="AI design prompt"
                ...
                {aiSvg&&!aiLoading&&(
                  <div style={{padding:'11px 13px',background:'rgba(16,185,129,0.05)',border:'1px solid rgba(16,185,129,0.15)',borderRadius:11,display:'flex',gap:10,alignItems:'center',marginBottom:14}}>
                    <div style={{width:46,height:46,background:'rgba(255,255,255,0.03)',borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                    ...
                    <div><div style={{fontSize:'0.68rem',color:'#10B981',fontWeight:700}}>✓ Design applied</div><div style={{fontSize:'0.62rem',color:'rgba(255,255,255,0.32)',marginTop:2}}>Showing on shirt</div></div>
                    <button aria-label="Remove" onClick={()=>setAiSvg(null)} style={{marginLeft:'auto',background:'none',border:'none',color:'rgba(255,255,255,0.2)',cursor:'pointer',fontSize:18}}>×</button>
                  </div>
                )}

                <div>
                  <div style={LS}>Inspiration prompts</div>
                  <div style={{display:'flex',flexDirection:'column',gap:4}}>
                    {['Minimalist mountain peak','Neon cyberpunk dragon','Bold street art typography','Abstract geometric waves','Vintage 80s sunset logo'].map(p=>(
                      <button key={p} onClick={()=>setAiPrompt(p)}
                        style={{padding:'8px 11px',borderRadius:9,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',color:'rgba(255,255,255,0.35)',fontSize:'0.72rem',fontWeight:500,cursor:'pointer',transition:'all 0.12s',textAlign:'left'}}
            ...
            {activeTool==='gfx'&&(
              <div style={{padding:'16px'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,marginBottom:16}}>
                  {EMOJIS.map(e=>(
                    <button key={e} aria-label={`Add ${e}`} onClick={()=>addGfx(e)}
                      style={{padding:'11px 2px',borderRadius:10,cursor:'pointer',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',fontSize:'1.35rem',transition:'all 0.12s',lineHeight:1}}
                      onMouseEnter={x=>{(x.currentTarget.style.background='rgba(0,229,200,0.1)');(x.currentTarget.style.transform='scale(1.15)');(x.currentTarget.style.borderColor='rgba(0,229,200,0.25)');}}
                ...
                {selLayer&&(
                  <div style={{background:'rgba(0,229,200,0.04)',border:'1px solid rgba(0,229,200,0.12)',borderRadius:11,padding:'12px'}}>
              ...
              <div style={{padding:'16px'}}>
                <div style={{marginBottom:20}}>
                    ...
                    {SHIRT_COLORS.map(c=>(
                      <button key={c.id} title={c.name} aria-label={`Color: ${c.name}`} aria-pressed={color.id===c.id} onClick={()=>setColor(c)}
                        style={{
                          aspectRatio:'1',
                          borderRadius:12,
                          border:`2px solid ${color.id===c.id?'#00E5C8':'rgba(255,255,255,0.06)'}`,
                          background:c.hex,
                          cursor:'pointer',
                          transition:'all 0.15s',
                          transform:color.id===c.id?'scale(1.06)':'scale(1)',
                          boxShadow:color.id===c.id?`0 0 20px ${c.hex}55, inset 0 1px 0 rgba(255,255,255,0.15)`:`inset 0 1px 0 rgba(255,255,255,0.12)`,
                          position:'relative',
                          display:'flex',
                          alignItems:'flex-end',
                        ...
                        {color.id===c.id&&<span style={{fontSize:'0.55rem',fontWeight:700,color:c.textColor,opacity:0.7,letterSpacing:'0.04em',lineHeight:1}}>✓</span>}
                      </button>
                    ))}
                  </div>
                  {/* Color names row */}
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
                    {SHIRT_COLORS.map(c=>(
                      <div key={c.id} style={{textAlign:'center',fontSize:'0.52rem',color:color.id===c.id?'rgba(0,229,200,0.8)':'rgba(255,255,255,0.18)',fontWeight:color.id===c.id?700:400,cursor:'pointer',letterSpacing:'0.02em',lineHeight:1.3}} onClick={()=>setColor(c)}>
                ...
                <div>
                  ...
                  <p style={{fontSize:'0.6rem',color:'rgba(255,255,255,0.18)',letterSpacing:'0.04em',lineHeight:1.6}}>Unisex · 100% ring-spun cotton · Pre-shrunk<br/>Standard fit · True to size</p>
                </div>
              </div>
            )}
            ...
            {activeTool==='order'&&(
              <div style={{padding:'16px'}}>
                {/* Status */}
                {!size&&(
                  <div style={{padding:'12px',background:'rgba(245,158,11,0.06)',border:'1px solid rgba(245,158,11,0.18)',borderRadius:12,fontSize:'0.72rem',color:'rgba(245,158,11,0.8)',marginBottom:14,lineHeight:1.5,display:'flex',gap:8,alignItems:'flex-start'}}>
                    <span>⚠</span><span>Please select a <button onClick={()=>setActiveTool('shirt')} style={{background:'none',border:'none',color:'#F59E0B',fontWeight:700,cursor:'pointer',textDecoration:'underline',padding:0,fontSize:'0.72rem'}}>shirt size</button> before ordering.</span>
                  </div>
                )}

                {/* Order summary card */}
                <div style={{background:'rgba(255,255,255,0.02)',borderRadius:12,border:'1px solid rgba(255,255,255,0.07)',padding:'13px 14px',marginBottom:14}}>
                  <div style={{fontSize:'0.58rem',fontWeight:700,color:'rgba(255,255,255,0.2)',letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:10}}>Summary</div>
                  {[
                    ['Custom shirt','$24.99'],
                    [`Shipping (Standard)`,`$${SHIPPING_PRICE.toFixed(2)}`],
                    ...(size?[[`${color.name} / Size ${size}`, '']]:[]),
                  ].map(([k,v])=>(
                    <div key={k} style={{display:'flex',justifyContent:'space-between',marginBottom:6,fontSize:'0.77rem'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',fontWeight:900}}>
                    <span>Total</span>
                    <span style={{color:'#00E5C8',fontSize:'1.05rem'}}>${total.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div style={{padding:'9px 11px',background:'rgba(245,158,11,0.04)',border:'1px solid rgba(245,158,11,0.1)',borderRadius:9,fontSize:'0.64rem',color:'rgba(245,158,11,0.5)',marginBottom:10}}>🔒 Stripe/PayPal coming soon — demo mode</div>
                {orderError&&<div role="alert" style={{padding:'9px 11px',background:'rgba(239,68,68,0.07)',border:'1px solid rgba(239,68,68,0.18)',borderRadius:9,fontSize:'0.7rem',color:'#f87171',fontWeight:600,marginBottom:10}}>⚠ {orderError}</div>}
              </div>
            )}
          ...
          <div style={{padding:'11px 13px',borderTop:'1px solid rgba(255,255,255,0.07)',background:'rgba(7,7,10,0.98)',backdropFilter:'blur(20px)',flexShrink:0,position:'relative'}}>
            ...
            {canOrder?(
              ...
              <button onClick={()=>setActiveTool(activeTool==='order'?activeTool:'order')}
      ...
      <style>{`
        ...
        @media(max-width:760px){
          .design-studio-grid{grid-template-columns:1fr!important;grid-template-rows:auto 1fr auto;overflow:auto!important;}
        }
      `}</style>
    </div>
  );
