...
function DesignStudio() {
      ...
      <svg width={w} height={h} viewBox={`0 0 ${SVG_W} ${SVG_H}`} fill="none">
        ...
        {interactive && layers.length===0 && !activeImg && !aiSvg && (
          <rect x={PRINT.x} y={PRINT.y} width={PRINT.w} height={PRINT.h}
            fill="rgba(255,255,255,0.02)" stroke="rgba(255,255,255,0.12)" strokeDasharray="3,2" rx="3" strokeWidth="0.7"/>
        )}
  ...
  if (ordered) return (
    <div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(180deg,#050507,#060610)',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{textAlign:'center',maxWidth:380,position:'relative',zIndex:1}}>
        ...
        <p style={{fontSize:'0.75rem',color:'rgba(255,255,255,0.3)',marginBottom:24,lineHeight:1.5}}>Track your order on your <Link href="/profile" style={{color:'#00E5C8',textDecoration:'none'}}>profile page</Link>.</p>
        <div style={{display:'flex',gap:10,justifyContent:'center'}}>
          <button onClick={()=>{setOrdered(false);setLayers([]);setAiSvg(null);setUploads({front:null,back:null,chest:null});}} style={{padding:'0.85rem 1.6rem',borderRadius:12,border:'1px solid rgba(255,255,255,0.12)',background:'rgba(255,255,255,0.05)',color:'rgba(255,255,255,0.7)',fontWeight:700,cursor:'pointer',fontSize:'0.85rem'}}>Design another</button>
          <Link href="/catalog" style={{padding:'0.85rem 1.6rem',borderRadius:12,background:'linear-gradient(135deg,#00E5C8,#0099FF)',color:'#050507',fontWeight:800,textDecoration:'none',display:'inline-flex',alignItems:'center',fontSize:'0.85rem'}}>Browse catalog →</Link>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:'#050507',color:'white',overflow:'hidden'}}>
      ...
      {/* Fullscreen overlay */}
      {fullscreen && (
        <div style={{position:'fixed',inset:0,zIndex:9999,background:'radial-gradient(ellipse at 50% 38%,rgba(18,18,28,1) 0%,rgba(4,4,6,1) 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',animation:'fsIn 0.25s ease'}}
          onClick={()=>setFullscreen(false)}>
          <div style={{filter:'drop-shadow(0 40px 80px rgba(0,0,0,0.9)) drop-shadow(0 0 60px rgba(0,229,200,0.06))'}}>
            <ShirtCanvas w={480} h={552} interactive={false}/>
          </div>
          <p style={{marginTop:28,color:'rgba(255,255,255,0.15)',fontSize:'0.68rem',letterSpacing:'0.12em',textTransform:'uppercase'}}>Tap anywhere to close</p>
          <button onClick={e=>{e.stopPropagation();setFullscreen(false);}} style={{position:'absolute',top:24,right:28,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'8px 18px',color:'rgba(255,255,255,0.5)',fontSize:'0.75rem',fontWeight:700,cursor:'pointer'}}>✕ Close</button>
        </div>
      )}

      {/* ── HEADER ──────────────────────────────────────────────── */}
      <header style={{height:54,flexShrink:0,borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',padding:'0 22px',gap:14,background:'rgba(5,5,7,0.96)',backdropFilter:'blur(24px)',position:'relative',zIndex:10}}>
        <div style={{position:'absolute',top:0,left:0,right:0,height:'1px',background:'linear-gradient(90deg,transparent 0%,rgba(0,229,200,0.45) 40%,rgba(0,153,255,0.3) 70%,transparent 100%)'}}/>

        <Link href="/catalog"
          style={{display:'flex',alignItems:'center',gap:5,color:'rgba(255,255,255,0.28)',fontSize:'0.7rem',textDecoration:'none',fontWeight:600,letterSpacing:'0.05em',transition:'color 0.15s',flexShrink:0}}
        ...
        <h1 style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontWeight:400,fontSize:'1.45rem',letterSpacing:'0.1em',lineHeight:1,margin:0,flex:1}}>
          DESIGN<span style={{color:'#00E5C8'}}>.</span>STUDIO
        </h1>

        <div style={{display:'flex',gap:7,alignItems:'center'}}>
          {layers.length>0 && (
            <span style={{background:'rgba(0,229,200,0.09)',border:'1px solid rgba(0,229,200,0.22)',borderRadius:20,padding:'3px 10px',fontSize:'0.6rem',fontWeight:700,color:'#00E5C8',letterSpacing:'0.07em'}}>
              {layers.length} LAYER{layers.length!==1?'S':''}
            </span>
          )}
          {size && (
            <span style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:20,padding:'3px 10px',fontSize:'0.6rem',fontWeight:700,color:'rgba(255,255,255,0.4)',letterSpacing:'0.06em',display:'flex',alignItems:'center',gap:5}}>
              <span style={{width:7,height:7,borderRadius:'50%',background:color.hex,display:'inline-block',boxShadow:`0 0 5px ${color.hex}99`}}/>
              {color.name} · {size}
            </span>
          )}
        </div>
        ...
        <button onClick={()=>setFullscreen(true)}
          style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:9,padding:'6px 13px',color:'rgba(255,255,255,0.38)',fontSize:'0.67rem',fontWeight:700,cursor:'pointer',letterSpacing:'0.06em',transition:'all 0.15s',flexShrink:0}}
          ...
          onMouseLeave={e=>{(e.currentTarget.style.background='rgba(255,255,255,0.04)');(e.currentTarget.style.borderColor='rgba(255,255,255,0.09)');(e.currentTarget.style.color='rgba(255,255,255,0.38)');}}>
          ⛶ PREVIEW
        </button>
      </header>

      {/* ── BODY ────────────────────────────────────────────────── */}
      <div className="design-body" style={{flex:1,display:'grid',gridTemplateColumns:'1fr 380px',overflow:'hidden',minHeight:0}}>

        {/* ── CANVAS ─────────────────────────────────────────────── */}
        <div style={{background:'radial-gradient(ellipse at 50% 30%,rgba(14,14,24,1) 0%,rgba(5,5,9,1) 100%)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}
          onClick={()=>setSelected(null)}>

          {/* Atmosphere */}
          <div style={{position:'absolute',width:560,height:560,borderRadius:'50%',background:'radial-gradient(circle,rgba(0,229,200,0.05) 0%,transparent 60%)',top:'-18%',right:'-8%',pointerEvents:'none'}}/>
          <div style={{position:'absolute',width:420,height:420,borderRadius:'50%',background:'radial-gradient(circle,rgba(0,100,255,0.04) 0%,transparent 65%)',bottom:'-10%',left:'-5%',pointerEvents:'none'}}/>
          <div style={{position:'absolute',width:280,height:280,borderRadius:'50%',background:'radial-gradient(circle,rgba(123,97,255,0.025) 0%,transparent 60%)',top:'35%',left:'8%',pointerEvents:'none'}}/>

          {/* Grid */}
          <div style={{position:'absolute',inset:0,pointerEvents:'none',opacity:0.025,backgroundImage:'linear-gradient(rgba(255,255,255,1) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,1) 1px,transparent 1px)',backgroundSize:'40px 40px'}}/>

          {/* Corner brackets */}
          <div style={{position:'absolute',top:14,left:14,width:22,height:22,borderTop:'1px solid rgba(0,229,200,0.18)',borderLeft:'1px solid rgba(0,229,200,0.18)',pointerEvents:'none'}}/>
          <div style={{position:'absolute',top:14,right:14,width:22,height:22,borderTop:'1px solid rgba(0,229,200,0.18)',borderRight:'1px solid rgba(0,229,200,0.18)',pointerEvents:'none'}}/>
          <div style={{position:'absolute',bottom:14,left:14,width:22,height:22,borderBottom:'1px solid rgba(0,229,200,0.18)',borderLeft:'1px solid rgba(0,229,200,0.18)',pointerEvents:'none'}}/>
          <div style={{position:'absolute',bottom:14,right:14,width:22,height:22,borderBottom:'1px solid rgba(0,229,200,0.18)',borderRight:'1px solid rgba(0,229,200,0.18)',pointerEvents:'none'}}/>

          {/* Front/Back toggle */}
          <div style={{position:'absolute',top:18,left:18,background:'rgba(5,5,9,0.88)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:11,padding:3,display:'flex',gap:2,backdropFilter:'blur(12px)'}}>
            {['Front','Back'].map(s=>(
              <button key={s} aria-pressed={(s==='Back')===showBack}
                onClick={e=>{e.stopPropagation();setShowBack(s==='Back');}}
                style={{padding:'5px 13px',borderRadius:8,border:'none',cursor:'pointer',background:(s==='Back')===showBack?'rgba(0,229,200,0.12)':'transparent',color:(s==='Back')===showBack?'#00E5C8':'rgba(255,255,255,0.28)',fontSize:'0.63rem',fontWeight:700,letterSpacing:'0.07em',transition:'all 0.15s'}}>
                {s.toUpperCase()}
              </button>
            ))}
          </div>
          ...
          <button onClick={e=>{e.stopPropagation();setFullscreen(true);}}
            style={{position:'absolute',top:18,right:18,background:'rgba(5,5,9,0.88)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:11,padding:'6px 13px',color:'rgba(255,255,255,0.38)',fontSize:'0.63rem',fontWeight:700,cursor:'pointer',backdropFilter:'blur(12px)',letterSpacing:'0.06em',transition:'all 0.15s'}}
            ...
            onMouseLeave={e=>{(e.currentTarget.style.background='rgba(5,5,9,0.88)');(e.currentTarget.style.borderColor='rgba(255,255,255,0.09)');(e.currentTarget.style.color='rgba(255,255,255,0.38)');}}>
            ⛶ FULLSCREEN
          </button>

          {/* Shirt */}
          <div ref={canvasRef} role="img" aria-label="Shirt design canvas — drag elements to position them"
            style={{filter:'drop-shadow(0 40px 80px rgba(0,0,0,0.75)) drop-shadow(0 0 50px rgba(0,229,200,0.04))',animation:'shirtIn 0.4s cubic-bezier(0.34,1.56,0.64,1)'}}>
            <ShirtCanvas w={360} h={414}/>
          </div>

          {/* Selected layer bar */}
          {selLayer && (
            <div style={{position:'absolute',bottom:20,background:'rgba(5,5,9,0.94)',border:'1px solid rgba(0,229,200,0.22)',borderRadius:12,padding:'7px 15px',display:'flex',alignItems:'center',gap:10,backdropFilter:'blur(14px)',fontSize:'0.68rem',animation:'fadeUp 0.15s ease',boxShadow:'0 4px 20px rgba(0,229,200,0.07)'}}>
              <span style={{color:'rgba(255,255,255,0.25)',fontSize:'0.58rem',letterSpacing:'0.1em'}}>SELECTED</span>
              <span style={{fontWeight:700,color:'#00E5C8',maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{selLayer.content}</span>
              <span style={{color:'rgba(255,255,255,0.12)'}}>·</span>
              <span style={{color:'rgba(255,255,255,0.22)',fontSize:'0.6rem'}}>Drag · Arrow keys · Del</span>
              <button onClick={()=>deleteLayer(selLayer.id)} style={{background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:6,padding:'3px 9px',color:'#f87171',fontSize:'0.6rem',fontWeight:700,cursor:'pointer'}}>✕</button>
            </div>
          )}

          {/* Empty hint */}
          {layers.length===0&&!activeImg&&!aiSvg&&!showBack&&(
            <div style={{position:'absolute',bottom:20,textAlign:'center',pointerEvents:'none',animation:'fadeUp 0.5s ease 0.3s both'}}>
              <p style={{color:'rgba(255,255,255,0.1)',fontSize:'0.65rem',letterSpacing:'0.12em',textTransform:'uppercase'}}>Add text · Upload · Generate with AI →</p>
            </div>
          )}
        </div>

        {/* ── RIGHT PANEL ────────────────────────────────────────── */}
        <div style={{borderLeft:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',background:'rgba(6,6,10,1)',overflow:'hidden'}}>
          ...
          <div style={{flex:1,overflowY:'auto',minHeight:0}}>
            ...
            <div style={{padding:'16px 16px 12px'}}>
              <div style={SHEAD}>
                <span style={{opacity:0.5}}>👕</span> SHIRT
              </div>

              {/* Color */}
              <div style={{marginBottom:14}}>
                <div style={LS}>Color <span style={{color:'#00E5C8',textTransform:'none',letterSpacing:0,fontWeight:500,fontSize:'0.7rem'}}>{color.name}</span></div>
                <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                  {SHIRT_COLORS.map(c=>(
                    <button key={c.id} title={c.name} aria-label={`Color: ${c.name}`} aria-pressed={color.id===c.id} onClick={()=>setColor(c)}
                      style={{width:32,height:32,borderRadius:'50%',border:'none',background:c.hex,cursor:'pointer',outline:color.id===c.id?'3px solid #00E5C8':'2px solid rgba(255,255,255,0.07)',outlineOffset:2,boxShadow:color.id===c.id?`0 0 14px ${c.hex}88`:'none',transition:'all 0.15s',transform:color.id===c.id?'scale(1.18)':'scale(1)'}}/>
                  ))}
                </div>
              </div>
              ...
              {/* Size */}
              <div>
                <div style={LS}>Size {size && <span style={{color:'#00E5C8',textTransform:'none',letterSpacing:0,fontWeight:500,fontSize:'0.7rem'}}>— {size}</span>}</div>
                <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                  {SHIRT_SIZES.map(s=>(
                    <button key={s} aria-pressed={size===s} onClick={()=>setSize(s)}
                      style={{width:44,height:44,borderRadius:10,cursor:'pointer',border:`1.5px solid ${size===s?'#00E5C8':'rgba(255,255,255,0.07)'}`,background:size===s?'rgba(0,229,200,0.1)':'rgba(255,255,255,0.02)',color:size===s?'#00E5C8':'rgba(255,255,255,0.32)',fontWeight:800,fontSize:'0.78rem',transition:'all 0.15s',transform:size===s?'scale(1.05)':'scale(1)',boxShadow:size===s?'0 0 12px rgba(0,229,200,0.14)':'none'}}>{s}</button>
                  ))}
                </div>
                <p style={{marginTop:6,fontSize:'0.57rem',color:'rgba(255,255,255,0.16)',letterSpacing:'0.04em'}}>Unisex · 100% ring-spun cotton · Pre-shrunk</p>
              </div>
            </div>
            <div style={DIV}/>
            ...
            <div style={{padding:'14px 16px 12px'}}>
              <div style={SHEAD}>
                <span style={{opacity:0.5}}>✏</span> DESIGN
              </div>

              {/* Tool tabs */}
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:3,background:'rgba(0,0,0,0.4)',borderRadius:11,padding:3,marginBottom:14}}>
                {([['text','✏','Text'],['upload','📁','Upload'],['ai','✦','AI'],['gfx','◆','Graphics']] as const).map(([id,icon,label])=>(
                ...
                <div style={{display:'flex',flexDirection:'column',gap:13}}>
                  <div>
                    ...
                    <div style={{display:'flex',gap:6}}>
                      <input aria-label="Text to add to design" value={textInput} onChange={e=>setTextInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addText()} placeholder="Type something..." maxLength={40}
                      ...
                      <button onClick={addText} disabled={!textInput.trim()}
                        style={{width:42,borderRadius:9,border:'none',background:textInput.trim()?'linear-gradient(135deg,#00E5C8,#0099FF)':'rgba(255,255,255,0.05)',color:textInput.trim()?'#050507':'rgba(255,255,255,0.15)',fontSize:'1.15rem',fontWeight:700,cursor:textInput.trim()?'pointer':'default',transition:'all 0.15s',flexShrink:0}}>+</button>
                    </div>
                    <div style={{display:'flex',flexWrap:'wrap',gap:4,marginTop:7}}>
                      {['YOUR NAME','EST. 2025','ORIGINAL','NO RULES'].map(t=>(
                        <button key={t} onClick={()=>setTextInput(t)}
                          style={{padding:'3px 9px',borderRadius:20,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',color:'rgba(255,255,255,0.28)',fontSize:'0.58rem',fontWeight:700,cursor:'pointer',letterSpacing:'0.04em',transition:'all 0.12s'}}
                  ...
                  <div>
                    ...
                    <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
                      {TEXT_COLORS.map(c=>(
                        <button key={c} aria-label={`Text color ${c}`} aria-pressed={textColor===c} onClick={()=>{setTextColor(c);if(selected)updateLayer(selected,{color:c});}}
                          style={{width:28,height:28,borderRadius:'50%',border:'none',background:c,cursor:'pointer',outline:textColor===c?'2.5px solid #00E5C8':'2px solid rgba(255,255,255,0.07)',outlineOffset:2,transition:'all 0.12s',transform:textColor===c?'scale(1.18)':'scale(1)'}}/>
                      ))}
                      <label aria-label="Custom text color" style={{width:28,height:28,borderRadius:'50%',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.05)',border:'1.5px solid rgba(255,255,255,0.1)',fontSize:'0.85rem',position:'relative'}}>
              ...
              {designTab==='upload' && (
                <div style={{display:'flex',flexDirection:'column',gap:12}}>
                  <div style={{display:'flex',gap:4}}>
                    {([['front','Front',uploads.front],['back','Back',uploads.back],['chest','Chest',uploads.chest]] as [UploadSlot,string,string|null][]).map(([slot,label,img])=>(
                      <button key={slot} aria-pressed={uploadSlot===slot} onClick={()=>setUploadSlot(slot)}
                        style={{flex:1,padding:'7px 4px',borderRadius:8,border:'1px solid',borderColor:uploadSlot===slot?'rgba(0,229,200,0.38)':'rgba(255,255,255,0.07)',background:uploadSlot===slot?'rgba(0,229,200,0.07)':'rgba(255,255,255,0.01)',color:uploadSlot===slot?'#00E5C8':'rgba(255,255,255,0.28)',fontSize:'0.62rem',fontWeight:700,cursor:'pointer',transition:'all 0.13s',display:'flex',alignItems:'center',justifyContent:'center',gap:4}}>
                  ...
                  <div onDragEnter={e=>{e.preventDefault();setFileDragging(true);}} onDragLeave={()=>setFileDragging(false)} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();setFileDragging(false);const f=e.dataTransfer.files[0];if(f)handleFile(f);}} onClick={()=>fileRef.current?.click()}
                    ...
                    {uploads[uploadSlot]
                      ? <div style={{display:'flex',alignItems:'center',gap:12,justifyContent:'center'}}><img src={uploads[uploadSlot]!} alt="upload" style={{height:54,maxWidth:100,borderRadius:6,objectFit:'contain'}}/><div style={{textAlign:'left'}}><div style={{fontSize:'0.7rem',color:'#10B981',fontWeight:700}}>✓ Uploaded</div><div style={{fontSize:'0.6rem',color:'rgba(255,255,255,0.25)',marginTop:2}}>Click to replace</div></div></div>
                      : <><div style={{fontSize:26,marginBottom:6}}>📁</div><div style={{fontWeight:700,color:'rgba(255,255,255,0.45)',fontSize:'0.78rem',marginBottom:4}}>Drop image here</div><div style={{fontSize:'0.62rem',color:'rgba(255,255,255,0.2)'}}>PNG · JPG · SVG — click to browse</div></>}
                  </div>
                  ...
                  {uploads[uploadSlot] && (
                    <button onClick={()=>setUploads(p=>({...p,[uploadSlot]:null}))} style={{padding:'8px',borderRadius:9,border:'1px solid rgba(239,68,68,0.18)',background:'rgba(239,68,68,0.06)',color:'#f87171',fontSize:'0.68rem',fontWeight:700,cursor:'pointer'}}>🗑 Remove image</button>
                  )}
                </div>
              )}
              ...
              {designTab==='ai' && (
                <div style={{display:'flex',flexDirection:'column',gap:12}}>
                  <div style={{background:'linear-gradient(135deg,rgba(0,229,200,0.05),rgba(0,153,255,0.04))',border:'1px solid rgba(0,229,200,0.1)',borderRadius:12,padding:'11px 13px'}}>
                    <div style={{fontSize:'0.58rem',color:'#00E5C8',fontWeight:700,letterSpacing:'0.1em',marginBottom:5}}>✦ AI DESIGN GENERATOR</div>
                    <div style={{fontSize:'0.7rem',color:'rgba(255,255,255,0.3)',lineHeight:1.55}}>Describe your idea and our AI will match the perfect design from the catalog.</div>
                  </div>

                  <div>
                    <div style={LS}>Describe your design</div>
                    <textarea value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} placeholder="e.g. A minimalist mountain with bold typography EXPLORE..." rows={4} maxLength={200} aria-label="AI design prompt"
                  ...
                  {aiLoading && (
                    <div style={{height:2,background:'rgba(255,255,255,0.05)',borderRadius:999,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${aiProgress}%`,background:'linear-gradient(90deg,#00E5C8,#0099FF)',borderRadius:999,transition:'width 0.2s'}}/>
                    </div>
                  )}
                  ...
                  {aiSvg && !aiLoading && (
                    <div style={{padding:'10px 12px',background:'rgba(16,185,129,0.05)',border:'1px solid rgba(16,185,129,0.15)',borderRadius:10,display:'flex',gap:10,alignItems:'center'}}>
                      <div style={{width:46,height:46,background:'rgba(255,255,255,0.02)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                      ...
                      <div>
                        <div style={{fontSize:'0.67rem',color:'#10B981',fontWeight:700}}>✓ Design ready</div>
                        <div style={{fontSize:'0.62rem',color:'rgba(255,255,255,0.32)',marginTop:2}}>Showing on shirt preview</div>
                      </div>
                      <button aria-label="Clear AI design" onClick={()=>setAiSvg(null)} style={{marginLeft:'auto',background:'none',border:'none',color:'rgba(255,255,255,0.2)',cursor:'pointer',fontSize:18}}>×</button>
                    </div>
                  )}
                    ...
                    <div style={{display:'flex',flexWrap:'wrap',gap:4}}>
                      {['Minimalist mountain','Neon dragon','Bold street art','Abstract waves','Vintage logo'].map(p=>(
                        <button key={p} onClick={()=>setAiPrompt(p)}
                          style={{padding:'4px 9px',borderRadius:20,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',color:'rgba(255,255,255,0.28)',fontSize:'0.6rem',fontWeight:600,cursor:'pointer',transition:'all 0.12s'}}
                ...
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  <div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:5}}>
                      {EMOJIS.map(e=>(
                        <button key={e} aria-label={`Add ${e} emoji`} onClick={()=>addGfx(e)}
                  ...
                  {selLayer && (
                    <div style={{background:'rgba(0,229,200,0.04)',border:'1px solid rgba(0,229,200,0.12)',borderRadius:10,padding:'10px 12px'}}>
                      ...
                      <input type="range" aria-label="Graphic size" min={12} max={80} value={selLayer.fontSize} onChange={e=>updateLayer(selLayer.id,{fontSize:+e.target.value})} style={{width:'100%',accentColor:'#00E5C8'}}/>
            ...
            {layers.length>0 && (
              <>
                <div style={{padding:'14px 16px 12px'}}>
                  <div style={SHEAD}>
                    <span style={{opacity:0.5}}>⊞</span> LAYERS ({layers.length})
                  </div>
            ...
            <div style={{padding:'14px 16px 4px'}}>
              <button onClick={()=>setShowOrderForm(p=>!p)}
                ...
                <div style={SHEAD}>
                  <span style={{opacity:0.5}}>🛒</span> ORDER
                  {(!size||!deliveryDone) && <span style={{marginLeft:8,fontSize:'0.55rem',color:'rgba(245,158,11,0.6)',fontWeight:600,letterSpacing:'0.04em',textTransform:'none'}}>— incomplete</span>}
                  {canOrder && <span style={{marginLeft:8,fontSize:'0.55rem',color:'rgba(0,229,200,0.6)',fontWeight:600,letterSpacing:'0.04em',textTransform:'none'}}>— ready ✓</span>}
                </div>
                <span style={{color:'rgba(255,255,255,0.18)',fontSize:'0.65rem',transition:'transform 0.2s',transform:showOrderForm?'rotate(180deg)':'rotate(0deg)',flexShrink:0}}>▼</span>
              </button>

              {showOrderForm && (
                <div style={{display:'flex',flexDirection:'column',gap:10,paddingBottom:8}}>
                  {(!color||!size) && (
                    <div style={{padding:'9px 12px',background:'rgba(245,158,11,0.05)',border:'1px solid rgba(245,158,11,0.16)',borderRadius:10,fontSize:'0.68rem',color:'rgba(245,158,11,0.75)',lineHeight:1.5}}>
                      ⚠ Select a shirt color and size above first
                    </div>
                  )}
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    <div><label htmlFor="ds-name" style={LS}>Full Name</label><input id="ds-name" aria-invalid={shipName.trim().length>0&&shipName.trim().length<2} style={INP} name="name" autoComplete="name" value={shipName} onChange={e=>setShipName(e.target.value)} placeholder="Jane Smith" maxLength={80}/></div>
                    <div><label htmlFor="ds-email" style={LS}>Email</label><input id="ds-email" aria-invalid={shipEmail.length>0&&!emailValid} style={INP} type="email" name="email" autoComplete="email" value={shipEmail} onChange={e=>setShipEmail(e.target.value)} placeholder="you@example.com" maxLength={120}/></div>
                  </div>
                  <div><label htmlFor="ds-street" style={LS}>Street Address</label><input id="ds-street" style={INP} name="street-address" autoComplete="street-address" value={shipStreet} onChange={e=>setShipStreet(e.target.value)} placeholder="123 Main St" maxLength={120}/></div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 60px 76px',gap:8}}>
                    <div><label htmlFor="ds-city" style={LS}>City</label><input id="ds-city" style={INP} name="city" autoComplete="address-level2" value={shipCity} onChange={e=>setShipCity(e.target.value)} placeholder="New York" maxLength={60}/></div>
                    ...
                    <div><label htmlFor="ds-zip" style={LS}>ZIP</label><input id="ds-zip" style={{...INP,fontFamily:'monospace'}} name="postal-code" autoComplete="postal-code" inputMode="numeric" value={shipZip} onChange={e=>setShipZip(e.target.value.replace(/\D/g,'').slice(0,5))} placeholder="10001"/></div>
                  </div>

                  <div style={{padding:'8px 10px',background:'rgba(245,158,11,0.04)',border:'1px solid rgba(245,158,11,0.1)',borderRadius:8,fontSize:'0.63rem',color:'rgba(245,158,11,0.5)',lineHeight:1.5}}>
                    🔒 Stripe/PayPal coming soon — demo mode active
                  </div>

                  {orderError && (
                    <div role="alert" aria-live="assertive" style={{padding:'8px 10px',background:'rgba(239,68,68,0.07)',border:'1px solid rgba(239,68,68,0.18)',borderRadius:8,fontSize:'0.7rem',color:'#f87171',fontWeight:600}}>
                      ⚠ {orderError}
                    </div>
                  )}
                </div>
            ...
            {/* Bottom padding so content isn't hidden under CTA */}
            <div style={{height:96}}/>
          </div>

          {/* ─── STICKY CTA ─────────────────────────────────── */}
          <div style={{padding:'12px 14px',borderTop:'1px solid rgba(255,255,255,0.07)',background:'rgba(5,5,7,0.97)',backdropFilter:'blur(20px)',flexShrink:0,position:'relative'}}>
            <div style={{position:'absolute',top:0,left:'15%',right:'15%',height:'1px',background:'linear-gradient(90deg,transparent,rgba(0,229,200,0.12),transparent)',transform:'translateY(-1px)'}}/>

            {canOrder ? (
              <button onClick={handleOrder} disabled={submitting}
                style={{width:'100%',padding:'14px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#00E5C8,#0099FF)',color:'#050507',fontWeight:800,fontSize:'0.9rem',cursor:submitting?'default':'pointer',boxShadow:'0 6px 24px rgba(0,229,200,0.28)',transition:'all 0.15s',opacity:submitting?0.7:1,letterSpacing:'0.02em'}}
                onMouseEnter={e=>{if(!submitting)(e.currentTarget.style.boxShadow='0 8px 32px rgba(0,229,200,0.42)');}}
                onMouseLeave={e=>{(e.currentTarget.style.boxShadow='0 6px 24px rgba(0,229,200,0.28)');}}>
              ...
              <button onClick={()=>setShowOrderForm(true)}
                ...
                onMouseEnter={e=>{(e.currentTarget.style.borderColor='rgba(255,255,255,0.14)');(e.currentTarget.style.color='rgba(255,255,255,0.4)');}}
                onMouseLeave={e=>{(e.currentTarget.style.borderColor='rgba(255,255,255,0.08)');(e.currentTarget.style.color='rgba(255,255,255,0.22)');}}>
                {!size ? '↑ Select color & size to continue' : !deliveryDone ? '↑ Fill in delivery details to order' : `Place Order — $${total.toFixed(2)}`}
              </button>
            )}
...
export default function DesignPage() {
  return (
    <Suspense fallback={<div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#050507',color:'rgba(255,255,255,0.18)',fontSize:'0.82rem',letterSpacing:'0.1em'}}>LOADING...</div>}>
      <DesignStudio/>
    </Suspense>
  );
}

const LS: React.CSSProperties    = {display:'block',fontSize:'0.58rem',fontWeight:700,color:'rgba(255,255,255,0.26)',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:5};
...
const DIV: React.CSSProperties   = {height:1,background:'linear-gradient(90deg,transparent,rgba(255,255,255,0.05),transparent)',margin:'0 0 0'};
