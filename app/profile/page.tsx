...
export default function ProfilePage() {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setProfileError(d.error ?? 'Failed to save'); return; }
      ...
      <main style={{ maxWidth: 660, margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
            <div><label style={lbl}>Street Address</label><input style={inp} value={addrStreet} onChange={e => setAddrStreet(e.target.value)} placeholder="123 Main St" /></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px', gap: 10 }}>
              <div><label style={lbl}>City</label><input style={inp} value={addrCity} onChange={e => setAddrCity(e.target.value)} placeholder="New York" /></div>
              <div>
