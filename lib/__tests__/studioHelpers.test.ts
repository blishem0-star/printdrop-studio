...
test('templates do not emit broken encoding artifacts', () => {
  const broken = /[גֲ‚€�™˜]/;
  for (const t of TEMPLATES) {
    const text = [t.id, t.name, t.cat, ...t.preview, ...t.build('#ffffff').map(l => l.content)].join(' ');
    assert.equal(broken.test(text), false, `${t.id} contains broken encoding`);
  }
});

test('svgToDataUrl is Unicode-safe', () => {
